...
test('scale2', async () => {
  const src = `
    import dkonasov__lygia::space::scale::scale2;
...
test('scale2_f', async () => {
  const src = `
    import dkonasov__lygia::space::scale::scale2_f;
    @compute @workgroup_size(1)
    fn foo() {
...
test('scale2dXY - matrix construction', async () => {
  const src = `
    import dkonasov__lygia::math::scale2d::scale2dXY;
    @compute @workgroup_size(1)
    fn foo() {
...
test('scale3', async () => {
  const src = `
    import dkonasov__lygia::space::scale::scale3;
    @compute @workgroup_size(1)
    fn foo() {
