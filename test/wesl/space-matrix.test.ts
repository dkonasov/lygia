...
test('tbn', async () => {
  const src = `
    import dkonasov__lygia::space::tbn::tbn;
...
test('perspective', async () => {
  const src = `
    ...
    import dkonasov__lygia::space::perspective::perspective;
    @compute @workgroup_size(1)
    fn foo() {
...
test('orthographic', async () => {
  const src = `
    import dkonasov__lygia::space::orthographic::orthographic;
    @compute @workgroup_size(1)
    fn foo() {
...
test('translate', async () => {
  const src = `
    import dkonasov__lygia::space::translate::translate;
    @compute @workgroup_size(1)
    fn foo() {
