...
test('cart2polar2', async () => {
  const src = `
    import dkonasov__lygia::space::cart2polar::cart2polar2;
...
test('polar2cart', async () => {
  const src = `
    import dkonasov__lygia::math::consts::{QTR_PI, SQRT2};
    import dkonasov__lygia::space::polar2cart::polar2cart;
    @compute @workgroup_size(1)
    fn foo() {
...
test('equirect2xyz', async () => {
  const src = `
    import dkonasov__lygia::space::equirect2xyz::equirect2xyz;
    @compute @workgroup_size(1)
    fn foo() {
...
test('xyz2equirect', async () => {
  const src = `
    import dkonasov__lygia::space::xyz2equirect::xyz2equirect;
    @compute @workgroup_size(1)
    fn foo() {
...
test('fisheye2xyz', async () => {
  const src = `
     import dkonasov__lygia::space::fisheye2xyz::fisheye2xyz;
...
test('nearest', async () => {
  const src = `
    import dkonasov__lygia::space::nearest::nearest;
    @compute @workgroup_size(1)
    fn foo() {
