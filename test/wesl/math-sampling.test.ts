...
test('hammersley', async () => {
  const src = `
    import dkonasov__lygia::math::hammersley::hammersley;
...
test('nyquist', async () => {
  const src = `
    import dkonasov__lygia::math::nyquist::nyquist;
    @compute @workgroup_size(1)
    fn foo() {
...
test('permute', async () => {
  const src = `
     import dkonasov__lygia::math::permute::permute;
...
test('grad4 - noise gradient helper', async () => {
  const src = `
    import dkonasov__lygia::math::grad4::grad4;
    @compute @workgroup_size(1)
    fn foo() {
...
test('hemisphereCosSample - unit vector property', async () => {
  const src = `
    import dkonasov__lygia::math::hammersley::hemisphereCosSample;
    @compute @workgroup_size(1)
    fn foo() {
