...
test('linearizeDepth', async () => {
  const src = `
    import dkonasov__lygia::space::linearizeDepth::linearizeDepth;
...
test('depth2viewZ perspective', async () => {
  const src = `
    import dkonasov__lygia::space::depth2viewZ::depth2viewZ;
    @compute @workgroup_size(1)
    fn foo() {
...
test('viewZ2depth perspective', async () => {
  const src = `
    import dkonasov__lygia::space::viewZ2depth::viewZ2depth;
    @compute @workgroup_size(1)
    fn foo() {
