...
test('eulerView', async () => {
  const src = `
    ...
    import dkonasov__lygia::space::eulerView::eulerView;
...
test('lookAt', async () => {
  const src = `
     import dkonasov__lygia::space::lookAt::lookAt;
     @compute @workgroup_size(1)
     fn foo() {
...
test('lookAtView', async () => {
  const src = `
     import dkonasov__lygia::space::lookAtView::lookAtView;
     @compute @workgroup_size(1)
     fn foo() {
...
test('lookAtViewRoll', async () => {
  const src = `
     import dkonasov__lygia::math::consts::HALF_PI;
     import dkonasov__lygia::space::lookAtView::lookAtViewRoll;
     @compute @workgroup_size(1)
     fn foo() {
...
test('lookAtViewFromDirection', async () => {
  const src = `
     import dkonasov__lygia::space::lookAtView::lookAtViewFromDirection;
     @compute @workgroup_size(1)
     fn foo() {
